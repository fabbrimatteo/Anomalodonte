...
class SimpleAutoencoder(BaseModel):
    ...
    @staticmethod
    def down_block(in_channels, out_channels, activation=nn.LeakyReLU(inplace=True)):
    ...
    @staticmethod
    def up_block(in_channels, out_channels, activation=nn.LeakyReLU(inplace=True)):
        return nn.Sequential(
            TConv2D(in_channels, out_channels, kernel_size=3, stride=2, padding=1, output_padding=1),
            nn.BatchNorm2d(out_channels),
            activation
        )
    ...
    def __init__(self, mid_channels=128, n_res_layers=2, code_channels=8, code_h=None, code_w=None):
        ...
        self.encoder = nn.Sequential(
            # --
            self.down_block(3, 16, activation=self.act),
            self.down_block(16, 32, activation=self.act),
            self.down_block(32, 64, activation=self.act),
            self.down_block(64, 128, activation=self.act),
            # --
        ...
        self.decoder = nn.Sequential(
            ...
            # --
            self.up_block(128, 64, activation=self.act),
            self.up_block(64, 32, activation=self.act),
            self.up_block(32, 16, activation=self.act),
            self.up_block(16, 8, activation=self.act),
            # --
